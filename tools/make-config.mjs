...
//                                    add --data-key <key> to keep using an existing database: the key is the
//                                    part after "nextgen-" in the nextgen-<key>.sqlite (or .jsonl) file name
...
const keyArg = process.argv.indexOf('--data-key');
const dataKey = keyArg > -1 ? String(process.argv[keyArg + 1] || '') : crypto.randomBytes(12).toString('hex');
if (!/^[a-f0-9]{8,64}$/.test(dataKey)) {
  console.error('--data-key must be the hex part of the nextgen-<key>.sqlite file name.');
  process.exit(1);
}
...
const NGS_DATA_KEY = '${dataKey}';
`, 'utf8');

console.log('Created public/api/config.php' + (keyArg > -1 ? ' (using the existing database)' : ''));
